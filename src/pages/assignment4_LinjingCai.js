...
//Hint: use d3.scaleBand for xScaleBar
    ...
    .domain(xData.map(d => d.station)) // Assuming each data point has a category
    .range([0, innerWidth]) // Assuming innerWidth is defined
    .padding(0.1);

    const yScaleBar = d3.scaleBand()
    .domain(xData.map(d => d.start)) // Assuming each data point has a category
    .range([innerHeightScatter, 0]) // Assuming innerWidth is defined
    .padding(0.1);
            ...
            <Row className='justify-content-md-center'>
                <Col>
                    <svg width={WIDTH} height={HEIGHT}>
                        ...
                        height={innerHeightScatter} width={innerWidth} handleStationHover={handleStationHover}
                        handleMouseOut={handleMouseOut} selectedStation={selectedStation} tooltipX={tooltipX} tooltipY={tooltipY}/>
                    </svg>
                </Col>
                <Col>
                    <svg width={WIDTH} height={HEIGHT}>
                        <BarChart offsetX={margin.left} offsetY={margin.top} data={data} xScale={xScaleBar} 
                        yScale={yScaleBar} height={innerHeightBar} width={innerWidth} handleStationHover={handleStationHover}
                        handleMouseOut={handleMouseOut} selectedStation={selectedStation}/>
                    </svg>
                </Col>
            </Row>
